...
_FIELDS = '''id, nome, tipo,
    cep, logradouro, numero, complemento, bairro, cidade, estado, endereco,
    latitude, longitude, status, proprietario_id, created_at'''
...
def add_ponto():
    ...
    partes = [data.get('logradouro'), data.get('numero'), data.get('bairro'),
              data.get('cidade'), data.get('estado')]
    endereco = ', '.join(p for p in partes if p) or data.get('endereco') or ''
